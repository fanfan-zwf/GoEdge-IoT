...
type KeyValue struct {
	Key   string `json:"key"`   // 对应Redis的key
	Value string `json:"value"` // 对应Redis的value（已转成字符串）
}

// BatchSetRedis 批量写入Redis
// 参数：data-要写入的数组
// 返回：error-错误信息
func Write_Batch(data []KeyValue) error {
	// 校验1：空数据直接返回
	if len(data) == 0 {
		return nil
	}
	...
	for idx, kv := range data {
		...
		// 校验3：value为空时给出警告（可选）
		if kv.Value == "" {
			fmt.Printf("警告：第%d个元素的value为空（key=%s）\n", idx, kv.Key)
		}
...
// BatchGetRedis 批量读取Redis
// 参数：ctx-上下文，keys-要读取的key数组
// 返回：KeyValue map[string]string，error-错误信息
func Read_Batch(keys []string) (keyvalue map[string]string, err error) {
	// 空 keys 直接返回空 map
	if len(keys) == 0 {
		return
	}
