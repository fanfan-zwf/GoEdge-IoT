...
func InitializeDrivers() (err error) {
	...
	for _, driveConfig := range driveConfigs {
		...
		// 2. 支持 → 才查询点位（你要的核心优化）
		fullConfig.Points, err = mqtt_rpc.Points_Config__Query([]uint{}, []uint{driveConfig.Id}, 0, 0)
		if err != nil {
		...
		case "Modbus_Tcp":
			...
			err = db_point.Write_value_Subscriber_mysqlconfig(fullConfig, modbus_tcp_struct.Write)
			if err != nil {
