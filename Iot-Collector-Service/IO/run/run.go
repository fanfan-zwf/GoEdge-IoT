...
// 	// 2. 查询驱动列表
// 	driveConfigs, err := mqtt_rpc.Drive_Config__Query([]uint{collectorInfo.Id}, []string{}, 0, 0)
// 	if err != nil {
// 		return fmt.Errorf("ERROR 驱动查询失败: %w", err)
// 	}

// 	// 优雅核心：驱动支持列表 + 初始化函数映射
// 	// driverInitializers := map[string]func(mysql.Drive_Config_type, []mysql.Points_Config_type) error{
...
// 		// 2. 支持 → 才查询点位（你要的核心优化）
// 		points, err := mqtt_rpc.Points_Config__Query([]uint{}, []uint{drive.Id}, 0, 0)
// 		if err != nil {
// 			return fmt.Errorf("ERROR 点位查询失败 driveId:%d: %w", drive.Id, err)
// 		}

// 		// 3. 自动调用对应驱动初始化
// 		err = initFunc(drive, points)
// 		if err != nil {
