...
var (
	Alarm_Config      map[Config_key_type]Alarm_Config_type
	Alarm_Config_RWMu sync.RWMutex
)

func init() {
	Alarm_Config = make(map[Config_key_type]Alarm_Config_type)
}

// 读取配置
func Alarm_Config__Query(key Config_key_type) (Alarm_Config_type, bool) {
	Alarm_Config_RWMu.RLock()
	defer Alarm_Config_RWMu.RUnlock()
	v, ok := Alarm_Config[key]
	return v, ok
}
func Alarm_Config__Query_list(keys []Config_key_type) (r []Alarm_Config_type) {
	for _, key := range keys {
		v, ok := Alarm_Config__Query(key)
		if ok {
			r = append(r, v)
		}
	}
...
// 修改状态
func Alarm_Config__Status_Update(key Config_key_type, status string, t time.Time) error {
...
func Alarm_Judgment(new fullConfig.Value_type) (Alarm_type, bool) {
	if new.DeviceId == "" {
		log.Printf("ERROR 获取报警配置失败: DeviceId=='' ")
	...
	cfg, ok := Alarm_Config__Query(key)
	if !ok {
		return Alarm_type{}, false
	}
	...
	Alarm_Config__Status_Update(key, r.Status, new.Time)
