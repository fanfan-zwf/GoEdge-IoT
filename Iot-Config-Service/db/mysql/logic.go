...
func Drive_Config__Query_DriveType(drive_type string) (configs []Drive_Config_type, err error) {
	...
	baseQuery := "SELECT `Id`, `Type`, `Name`, `Config`, `Points_Length`, `Collector_Id` FROM `Drive_Config` WHERE `Type` = ?"
	// 4. 执行查询（统一处理，减少重复代码）
...
func Drive_Config__Query_DriveId(driveid uint) (config Drive_Config_type, err error) {
	...
	// 1. 初始化SQL和参数切片，避免多次拼接字符串，提升可读性和安全性
	baseQuery := "SELECT `Id`, `Type`, `Name`, `Config`, `Points_Length`, `Collector_Id` FROM `Drive_Config` WHERE `Id` = ?"
...
func Drive_Config__Query(collectorId uint, driveType string, page uint, pageSize uint) (configs []Drive_Config_type, err error) {
	...
	// 1. 初始化SQL和参数切片，避免多次拼接字符串，提升可读性和安全性
	baseQuery := "SELECT `Id`, `Type`, `Name`, `Config`, `Points_Length`, `Collector_Id` FROM `Drive_Config`"
...
func Drive_Config__Add(configs ...Drive_Config_type) (err error) {
	...
	for i, cfg := range configs {
		if cfg.Id != 0 || cfg.Points_Length != 0 || cfg.Collector_Id != 0 {
			err = fmt.Errorf("批量新增失败：第%d条配置参数错误 Id=%d/Points_Length=%d/Collector_Id=%d, 必须为0 ",
				i, cfg.Id, cfg.Points_Length, cfg.Collector_Id)
			return
		}
...
func Drive_Config__Update(configs ...Drive_Config_type) (err error) {
	...
	for idx, config := range configs {
		...
		if config.Collector_Id != 0 {
			err = fmt.Errorf("ERROR 第%d条配置Collector_Id不允许手动赋值 值:%d", idx, config.Collector_Id)
			return
		}

		// 可选：校验必填字段（Type/Name/Config非空，根据业务需求加）
		if config.Type == "" || config.Name == "" {
			err = fmt.Errorf("批量新增失败：第%d条配置Type/Name不能为空", idx)
			return
		}
