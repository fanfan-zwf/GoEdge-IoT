...
func (a Access_Token_redis) Write_Key_list(param any) error {
	panic("unimplemented")
}
...
func User_Status(User_Access_Token string) (r User_Status__type, err error) {
...
// 缓存用户登陆状态
func Read_Cache_User_status(User_Access_Token string) (result Access_Token_redis, err error) {
	var read_key string

	// 使用包别名 r 调用 Read_Key
	read_key, err = r.Read_Key(User_Access_Token)

	// 修复点：此时 r 指代包，r.Nil 指代 redis.Nil
	if err == r.Nil {
		var user_status User_Status__type
		user_status, err = User_Status(User_Access_Token)
		if err != nil {
...
// 缓存 API 登陆状态 (同样修复此处的同名问题)
func Read_Cache_Api_status(Api_Access_Token string) (result Api_Access_Token_redis, err error) {
	var read_key string
	read_key, err = r.Read_Key(Api_Access_Token)

	// 修复点：同上
	if err == r.Nil {
		var api_status Api_Status__type
		api_status, err = Api_Status(Api_Access_Token)
		if err != nil {
			log.Println("ERROR API 请求失败：", err)
			return
		}
		...
		if ttlDuration <= 0 {
			log.Printf("WARN: 令牌已过期 Token: %s", Api_Access_Token)
			err = r.Nil
			return
		}
