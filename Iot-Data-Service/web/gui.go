...
func Points_Config__Query(ctx *gin.Context) {
	...
	var configs []db_mysql.Points_Config_type
	configs, err = db_mysql.Points_Config__Query(jsondata.Drive_Id, jsondata.Page, jsondata.Page_Size)
	if err != nil {
		ctx.Set("Response", []any{520, err.Error()})
		return
	}
...
func Points_Config__Add(ctx *gin.Context) {
	var jsondata db_mysql.Points_Config_type
	err := ctx.BindJSON(&jsondata)
	if err != nil {
		ctx.Set("Response", []any{417, "请求格式不对"})
		return
...
func gui_api(r *gin.Engine) {
	...
	r.GET("/gui/v1.0/config/points/count", Points_Config__Count)   // 点位-》查询数量
	r.GET("/gui/v1.0/config/points/query", Points_Config__Query)   // 点位-》查询配置
	r.GET("/gui/v1.0/config/points/add", Points_Config__Add)       // 点位-》增加
	r.GET("/gui/v1.0/config/points/update", Points_Config__Update) // 点位-》更新
	r.GET("/gui/v1.0/config/points/del", Points_Config__Del)       // 点位-》删除
}
